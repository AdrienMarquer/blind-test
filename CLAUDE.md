...
**Note**: SQLite database is stored at `./data/blind-test.db` by default (or configure via `DATABASE_URL` env var). WAL mode is enabled for better concurrent read performance.
...
3. **Repository Pattern**: The server uses repository classes (RoomRepository, PlayerRepository, SongRepository) for data access with SQLite via Drizzle ORM.
...
- **Database**: SQLite with WAL mode (no external dependencies)
- **ORM**: Drizzle ORM for type-safe database access
- **Migrations**: Managed by Drizzle Kit (`drizzle/` folder)
- **Location**: `./data/blind-test.db` by default (configurable via `DATABASE_URL`)
- **Files**: Main `.db` file + `.db-shm` and `.db-wal` (WAL mode temporary files)
...
3. Update Drizzle schema in `apps/server/src/db/schema.ts`
