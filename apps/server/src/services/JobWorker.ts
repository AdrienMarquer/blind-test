...
export class JobWorker {
	...
	private async processYouTubeDownload(job: Job): Promise<void> {
		...
		const song = await songRepository.create({
			...
			duration: metadata.duration,
			clipStart: (clipStart !== undefined && clipDuration !== undefined) ? 0 : (clipStart ?? SONG_CONFIG.DEFAULT_CLIP_START),
		...
		for (let i = 0; i < videos.length; i++) {
			...
			try {
				...
				const song = await songRepository.create({
					...
					genre: finalMetadata.genre,
					duration: metadata.duration,
					clipStart: (video.clipStart !== undefined && video.clipDuration !== undefined) ? 0 : (video.clipStart ?? SONG_CONFIG.DEFAULT_CLIP_START),
